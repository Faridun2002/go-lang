...
func FunUpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	for search.Next() {
		...
		update, err := db.Query(fmt.Sprintf("UPDATE user SET name='%s', surname='%s', login='%s', email='%s', password='%s' WHERE id!=0 AND login='%s' LIMIT 1", Name_user, Surname_user, Login_user, Email_user, Password_user, UpdateUserName))
		pkg.ForError(err)
		defer update.Close()
